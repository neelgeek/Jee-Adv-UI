 ...
 jQuery(document).ready(function() {
     ...
     mockWaterMark = Math.floor(Math.random() * 10000000000000); //Random 13 digit number
         ...
         jQuery.ajax({
             ...
             success: function(data) {
                 ...
                 if (validity.toLowerCase() != "live") {
                     window.location.href = "error.html?E105" + (($.cookie("isApp") == "true") ? "isApp" : "") + (($.cookie("isLauncher") == "true") ? "isLauncher" : "");
                 }
                 if (url.indexOf("index.html") >= 0)
 ...
 jQuery(document).ready(function() {
     ...
     if (typeof(sessionStorage.cand_master_id) != "undefined") {
         ...
         $.removeCookie("isApp");
         $.removeCookie("isLauncher");
         $.removeCookie("timeZone");
         $.removeCookie("xmlFilePath");
         ...
         $.cookie("isApp", sessionStorage.isApp);
         $.cookie("isLauncher", sessionStorage.isLauncher);
         $.cookie("timeZone", sessionStorage.timeZone);
         $.cookie("xmlFilePath", sessionStorage.xmlFilePath);
     }
     $.ajaxSetup({
         async: true
     ...
     if (url.indexOf("quiz.html") >= 0 || url.indexOf("instructions.html") >= 0 || url.indexOf("FeedBack.html") >= 0 || url.indexOf("close.html") >= 0) {
         ...
         if (assessmentId == null || assessmentId.length == 0 || orgId == null || orgId.length == 0) {
             window.location.href = "error.html" + (($.cookie("isApp") == "true") ? "isApp" : "") + (($.cookie("isLauncher") == "true") ? "isLauncher" : "");
         }
     }
     ...
     $.ajax({
         ...
         success: function(data) {
             if (data != null && data.Error != null) {
                 window.location.href = "error.html?E103" + (($.cookie("isApp") == "true") ? "isApp" : "") + (($.cookie("isLauncher") == "true") ? "isLauncher" : "");
 ...
 function readAndReturnXML(filePath) {
     ...
     $.ajax({
         ...
         //  error: function() {
         //      setTimeout(function() {
         //          window.location.href = "error.html?E404" + (($.cookie("isApp") == "true") ? "isApp" : "") + (($.cookie("isLauncher") == "true") ? "isLauncher" : "");
         //      }, 50);
         //  }
     });
     return xml;
         ...
         $(sysInstrXML).find("INSTRUCTION").each(function() {
             if (langName.toUpperCase() == $(this).find("LANGNAME").text().toUpperCase()) {
                 ...
                 if (isMarkedForReviewConsidered == "YES") {
                     $("iframe").contents().find('.review_answered').css({
 ...
 function basicDetails(xml, isExtraJSRequired, page) {
     ...
     if ($(xml).find('StoreCandResponse').length > 0 && $(xml).find('StoreCandResponse').text() == 'YES') {
         ...
         mockVar.timeZone = $.cookie("timeZone") != undefined ? $.cookie("timeZone") : "";
         mockVar.xmlFilePath = $.cookie("xmlFilePath") != undefined ? $.cookie("xmlFilePath") : "";
     }
 ...
 function candidateTotalTimeSpent() {
     setTimeout(function() {
         mockVar.TotalTimeSpentInQuizPage = mockVar.TotalTimeSpentInQuizPage + 1;
         candidateTotalTimeSpent();
     }, 1000);
 }
 ...
 function readSysInstructionsXMLQuizPage(QPxml, xml, orgId, mockId, isResumed) {
     ...
     mockVar.TotalTimeSpentInQuizPage = (typeof(mockVar.TotalTimeSpentInQuizPage) != "undefined" ? mockVar.TotalTimeSpentInQuizPage : 0);
     mockVar.AttemptStartTime = (typeof(mockVar.AttemptStartTime) != "undefined" ? mockVar.AttemptStartTime : (typeof(sessionStorage.startTime) != "undefined" ? sessionStorage.startTime : "NA"));
     mockVar.loginId = (typeof(mockVar.loginId) != "undefined" ? mockVar.loginId : "NA");
     mockVar.emailId = (typeof(mockVar.emailId) != "undefined" ? mockVar.emailId : "NA");
     mockVar.courseId = typeof(sessionStorage.courseId) != "undefined" ? sessionStorage.courseId : "NA";
     setTimeout(function() {
         candidateTotalTimeSpent();
     }, 1000);
     var counter = 1;
...
var innerDoc = iframe.contentDocument || iframe.contentWindow.document;
                 ...
                 $(this).find("question").each(function() {
                     if (noOfTypingQues == 0) {
                         ...
                         if (quesType == "MCQ" || quesType == "MSQ") {
                                     ...
                                     if (mockVar.MagnifyingGlass == 1) {
                                         ...
                                         if (optText.indexOf('<img') != -1) {
                                             ...
                                             for (var i = 0; i < newImageFile.length; i++) {
                                                 while (optText.indexOf(newImageFile[i]) != -1) {
                                                     var imageName = newImageFile[i].split('src=')[1].split('/>');
                                                     optText = optText.replace(newImageFile[i], "<span class='ans'><span class='zoomimage' ><a target='_blank'><img src=" + imageName[0] + " class='max_img_ico' title='" + $(globalXmlvar).find('clickImageToZoom').text() + "'></a></span></span>");
                                                     oldFileCounter++;
                                                 }
                                             }
                                         }
                             ...
                             $(this).find('languageSpecificData').find('lang').each(function() {
                                 langId = $(this).attr('id');
                                 quesText = ($(this).find('questionString').text()).replace(/&#39;/g, "'");
                                 hint = $(this).find('hint').text() != "NA" ? $(this).find('hint').text() : "";
                                 if (hint == "" || hint == "NA")
                                     showHintDiv = showHintDiv || false;
                                 else
 ...
 function readXMLQuestionPaper(xml, isResumed) {
     ...
     if (parseInt(isResumed) == 0) {
         ...
         var compreId = '',
             ...
             quesText = '',
             hint = '',
             groupComprehensionQuestions = '',
             groupLAQQuestions = '';
         var allowedProgression = '',
             noOfReplays = '',
             isControlEnable = '',
             autoplay = '',
             additionalQuesType = '',
             columnType = '',
             subquestionShuffling = '';
                 ...
                 if (mockVar.MagnifyingGlass == 1) {
                     ...
                     if (quesText.indexOf('<img') != -1) {
                         ...
                         for (var i = 0; i < newImageFile.length; i++) {
                             while (quesText.indexOf(newImageFile[i]) != -1) {
                                 var imageName = newImageFile[i].split('src=')[1].split('/>');
                                 quesText = quesText.replace(newImageFile[i], "<span class='ans'><span class='zoomimage' ><a target='_blank'><img src=" + imageName[0] + " class='max_img_ico' title='" + $(globalXmlvar).find('clickImageToZoom').text() + "'></a></span></span>");
                                 oldFileCounter++;
                             }
                         }
                     }
         ...
         if ($(xml).find("group").length > 0) {
             ...
             $(xml).find("group").each(function() {
                 ...
                 if (mockVar.groups[counter].instructionTime != 0)
                     mockVar.groups[counter].enableInstruView = $(this).attr("revisitAllowedForInstructions");
                 else
                     mockVar.groups[counter].enableInstruView = "false";
         ...
         for (var i = 0; i < mockVar.groups.length; i++) {
             for (var j = 0; j < mockVar.groups[i].secDetails.length; j++) {
                 if (typeof(mockVar.groups[i].secDetails[j].groupAllQuestions) != "undefined" && mockVar.groups[i].secDetails[j].groupAllQuestions == "true") {
 ...
 function fillQuesContent() {
     ...
     mockVar.curSectionQuestions = iOAP.secDetails[iOAP.curSection].questions;
     if (iOAP.secDetails[iOAP.curSection].showMarkedForReview) {
         $(".review_mark").show();
         $(".review_answer").show();
         $(".MarkForReviewDiv").show();
         ...
         if (mockVar.curQuesBean.quesType == "SA" || mockVar.curQuesBean.quesType == "LA@@SA" || mockVar.curQuesBean.quesType == "COMPREHENSION@@SA") {
             ...
             if (mockVar.curQuesBean.comprehensionId == 0 && mockVar.curQuesBean.laqId == 0) {
                 ...
                 if (mockVar.curQuesBean.keyboardType == 'Numeric') {
                     ...
                     if ((mockVar.curQuesBean.quesLangBeans[mockVar.langIndex].quesText.indexOf('<ResponsePlaceHolder>') == -1)) {
                         triggerKeyboard(1);
                     }
                 ...
                 // alphanumeric textarea
                     ...
                     wordCountCheck();
                     var quesIdToAppend = mockVar.curQuesBean.quesLangBeans[mockVar.langIndex].quesText.indexOf('<ResponsePlaceHolder>') == -1 ? '' : mockVar.curQuesBean.quesId;
                     setCursorPos($("#answer" + quesIdToAppend), $('#answer' + quesIdToAppend).val().length);
                     $('#clearResponse').hide();
                     if (iOAP.secDetails[iOAP.curSection].isOptional == 'true' && !iOAP.secDetails[iOAP.curSection].isSelected) {
                         $('#answer').attr('disabled', 'disabled');
                     } else {
                         ...
                         if (iOAP.secDetails[iOAP.curSection].isOptional == 'true' && !iOAP.secDetails[iOAP.curSection].isSelected) {
                             ...
                             if (iOAP.secDetails[iOAP.curSection].questions[curGroupCompreQues].quesType == "MCQ")
                                 ...
                                 if (iOAP.secDetails[iOAP.curSection].questions[curGroupCompreQues].keyboardType == 'Alphanumeric') {
                                     ...
                                     $('#numericKeyBoardDiv' + iOAP.secDetails[iOAP.curSection].questions[curGroupCompreQues].quesId).remove();
                                     if ((iOAP.secDetails[iOAP.curSection].questions[curGroupCompreQues].quesLangBeans[mockVar.langIndex].quesText.indexOf('<ResponsePlaceHolder>') == -1) && (additionalQuesType == undefined || additionalQuesType == "" || additionalQuesType !== "FITB")) {
         ...
         for (var k = 0; k < mockVar.curSectionQuestions.length; k++) {
             if (mockVar.curSectionQuestions[k].quesType == "SA" || mockVar.curSectionQuestions[k].quesType == "LA@@SA" || mockVar.curSectionQuestions[k].quesType == "COMPREHENSION@@SA") {
                 if (mockVar.curSectionQuestions[k].keyboardType == 'Numeric') {
                     ...
                     if ((mockVar.curSectionQuestions[k].quesLangBeans[mockVar.langIndex].quesText.indexOf('<ResponsePlaceHolder>') == -1)) {
 ...
 function fillMCQQuesGroup(quesTxt, quesOpts, answer, curQuestion, qNo, quesId) {
     ...
     hasPlaceHolder = quesTxt.indexOf('<ResponsePlaceHolder>') != -1 ? true : false;
     if (typeof(curQuestion.singleLineQuestionOption) == "undefined" || curQuestion.singleLineQuestionOption == "" || curQuestion.singleLineQuestionOption == "false") {
         str += "<div class='groupQuestion textHighlighter " + textHighlighterGrpSecQuesId + " " + quesId + "_QuesId '  style='width:98%;" + ((!hasPlaceHolder) ? "margin-left:5px;" : "") + "font-family:Arial,verdana,helvetica,sans-serif;padding-bottom:10px;padding-top:10px;'>";
         if (typeof(curQuestion.displayQuestionNo) == "undefined" || curQuestion.displayQuestionNo == "" || curQuestion.displayQuestionNo == "true")
             str += "<span>" + (qNo + 1) + "</span>.";
         str += (hasPlaceHolder ? quesTxt.replace(/<ResponsePlaceHolder>/g, fillBlankMCQ('', '', quesOpts, answer, curQuestion, curQuestion.quesId, true)) : quesTxt) + "</div>";
         if (!hasPlaceHolder) {
         ...
         str += "<div class='groupQuestion textHighlighter " + textHighlighterGrpSecQuesId + " " + quesId + "_QuesId'  style='width:98%;" + ((!hasPlaceHolder) ? "margin-left:5px;" : "") + "font-family:Arial,verdana,helvetica,sans-serif;padding-bottom:10px;padding-top:10px;white-space:nowrap;'>";
         if (curQuestion.displayQuestionNo == "true" || curQuestion.displayQuestionNo == "" || typeof(curQuestion.displayQuestionNo) == "undefined")
             str += "<span>" + (qNo + 1) + "</span>.";
         str += (hasPlaceHolder ? quesTxt.replace(/<ResponsePlaceHolder>/g, fillBlankMCQ('', '', quesOpts, answer, curQuestion, curQuestion.quesId, true)) : quesTxt);
         if (!hasPlaceHolder) {
             var answers = answer.split(",");
 ...
 function fillMCQQues(quesTxt, quesOpts, answer) {
     ...
     if (mockVar.curQuesBean.singleLineQuestionOption == "false" || mockVar.curQuesBean.singleLineQuestionOption == "" || typeof(mockVar.curQuesBean.singleLineQuestionOption) == "undefined") {
         str += "<div class='textHighlighter " + textHighlighterGrpSecQuesId + " " + (mockVar.curQuesBean.quesId) + "_QuesId'  style='width:98%;margin-left:5px;font-family:Arial,verdana,helvetica,sans-serif;padding-bottom:10px;padding-top:10px;'> " + (hasPlaceHolder ? quesTxt.replace(/<ResponsePlaceHolder>/g, fillBlankMCQ('', '', quesOpts, answer, mockVar.curQuesBean, mockVar.curQuesBean.quesId, true)) : quesTxt) + "</div>";
         if (!hasPlaceHolder) {
         ...
         str += "<div class='textHighlighter " + textHighlighterGrpSecQuesId + " " + (mockVar.curQuesBean.quesId) + "_QuesId'  style='width:98%;margin-left:5px;font-family:Arial,verdana,helvetica,sans-serif;padding-bottom:10px;padding-top:10px;white-space:nowrap'> " + (hasPlaceHolder ? quesTxt.replace(/<ResponsePlaceHolder>/g, fillBlankMCQ('', '', quesOpts, answer, mockVar.curQuesBean, mockVar.curQuesBean.quesId, true)) : quesTxt);
         if (!hasPlaceHolder) {
             var answers = answer.split(",");
             str += ' <span class="singleLineOptions">';
             for (var i = 0; i < quesOpts.length; i++) {
 ...
 function fillSAQuesGroup(quesTxt, answer, curQuestion, qNo, quesId) {
     ...
     if (curQuestion.displayQuestionNo == "true" || curQuestion.displayQuestionNo == "" || typeof(curQuestion.displayQuestionNo) == "undefined")
         str += "	<span>" + (qNo + 1) + "</span>.";
     str += (hasPlaceHolder ? quesTxt.replace(/<ResponsePlaceHolder>/g, fillBlankSA('', '', answer, curQuestion, curQuestion.quesId, true)) : quesTxt) + "</div>";
     if (!hasPlaceHolder) {
 ...
 function fillSAQues(quesTxt, answer) {
     ...
     var str = "<div id='quesAnsContent' class='Ans_Area'>";
     if (mockVar.curQuesBean.keyboardType != "AudioFile" && mockVar.curQuesBean.keyboardType != "VideoFile")
         str += "<div class='textHighlighter " + textHighlighterGrpSecQuesId + " " + (mockVar.curQuesBean.quesId) + "_QuesId'  style='width:98%;margin-left:5px;font-family:Arial,verdana,helvetica,sans-serif;padding-bottom:10px;padding-top:10px'> " + (hasPlaceHolder ? quesTxt.replace(/<ResponsePlaceHolder>/g, fillBlankSA('', '', answer, mockVar.curQuesBean, mockVar.curQuesBean.quesId, true)) : quesTxt) + "</div>";
     if (!hasPlaceHolder)
 ...
 function fillFITBQues(compQues) {
     ...
     while (FITBParam !== null) {
         ...
         if (gapType.toUpperCase() === "MCQ" && gapId === curSection.questions[curQues].gapId) {
             compTempQues = fillBlankMCQ(compTempQues, replaceStr, curSection.questions[curQues].options, curSection.questions[curQues].quesParam.selectedOptId, curSection.questions[curQues], gapId, false);
         } else if (gapType.toUpperCase() === "SA" && gapId === curSection.questions[curQues].gapId) {
             compTempQues = fillBlankSA(compTempQues, replaceStr, curSection.questions[curQues].answer, curSection.questions[curQues], gapId, false);
         }
 ...
 function fillBlankMCQ(compQuesStr, replaceStr, quesOpts, answer, curQuestion, gapId, isSingleFITB) {
     var str = "";
     str += '<span class="select-style  FITBQuestionList ' + gapId + '_FITBM';
     if (isSingleFITB)
         str += ' SingleFITB';
     str += '"><select id="answers' + curQuestion.quesId + '">';
     var answers = answer.split(",");
 ...
 function fillBlankSA(compQuesStr, replaceStr, answer, curQuestion, gapId, isSingleFITB) {
     ...
     if (curQuestion.rows == 'undefined' || curQuestion.columns == 'undefined' || (curQuestion.rows == 10 && curQuestion.columns == 100) || (curQuestion.rows == 10 && curQuestion.columns == 70)) {
         str += '<input class="keyboardInput SAAnswer" type="text" id="answer' + curQuestion.quesId + '" style="height:30px;" class="answer" onpaste="return false;" ondrop="return false;" onkeydown="disableTab(event); allowSAInputsForMultiLang(event); alphaWordLimitGroup(event,' + curQuestion.quesId + '); " onkeyup="word_countGroup(' + curQuestion.quesId + ');alphaWordLimitGroup(event,' + curQuestion.quesId + ');" onmousedown="alphaWordLimitGroup(32,' + curQuestion.quesId + ')"  value="' + curQuestion.quesParam.answer + '" ';
         if (curQuestion.keyboardType == "Numeric") {
 ...
 function submitGroup() {
     ...
     if (typeof(mockVar.groups[mockVar.currentGrp].instructionTime) != 'undefined' && mockVar.groups[mockVar.currentGrp].instructionTime != 0 && mockVar.groups[mockVar.currentGrp].instru.length > 0) {
         ...
         if (mockVar.remainingInstruTime > 0 && mockVar.resumedFromInstruPage == 1 && mockVar.groupAtInterruption == mockVar.currentGrp) {
             ...
             moveToGroupFromGrpInst();
         } else if (mockVar.resumedFromInstruPage == 1 && mockVar.groupAtInterruption != mockVar.currentGrp) {} else {
             instruTimeCounter(mockVar.timeCounter);
             showGrpInstructions();
         }
     } else {
 ...
 function fillNumberPanel() {
     ...
     if (mockVar.MagnifyingGlass != undefined && mockVar.MagnifyingGlass == 1) {
         ...
         $('.imageZoom').jqzoom(options).click(function(event) {
             event.preventDefault(); //Preventing selection of options on image click
 ...
 function resetOption() {
     ...
     if (mockVar.curQuesBean.quesType == "SA" || mockVar.curQuesBean.quesType == "COMPREHENSION@@SA" || mockVar.curQuesBean.quesType == "LA@@SA") {
         $('#answer' + quesIdToAppend).val('');
         $("#noOfWords").text('');
     } else {
         if (quesIdToAppend == '' || (mockVar.curQuesBean.quesType.indexOf('MSQ') != -1)) {
             ...
             $('#answers' + quesIdToAppend).val('select')
         }
     }
     fnSubmit('RESET');
 }
 ...
 function submitConfirmation(param) {
     ...
     if (param == 'submit') {
         ...
         timeOutStr += "<div  onclick='submitMock()' type='button' class='btn btn-primary nextbtn'><span style='line-height: 33px;'><span id='timeoutSummaryNextBtnDiv '><strong>Next</strong></span></span></div></div>";
         //<img id='forwordImage' src='images/Forward-25.png' width='25' height='25' />
         return timeOutStr;
     } else if (param == 'break') {
         //		for(var j=0;j<mockVar.groups.length;j++)
 ...
 function moveToFeedback() {
     ...
     if (mockVar.isFeedBackRequired == "NO") {
         window.location.href = "close.html?" + mockVar.orgId + "@@" + mockVar.mockId + "@@" + mockVar.subscribedFor + "#" + (($.cookie("isApp") == "true") ? "isApp" : "") + (($.cookie("isLauncher") == "true") ? "isLauncher" : "");
     } else {
         ...
         if (action != "SKIP") {
             for (var i = 0; i < section.questions.length; i++) {
                 if (section.questions[i].quesType == "SA" || section.questions[i].quesType == "COMPREHENSION@@SA" || section.questions[i].quesType == "LA@@SA") {
                     ...
                     selectedAnswer = "";
                     selectedOptId = "";
                     if (section.questions[i].quesType.indexOf("MSQ") != -1) {
                     ...
                     } else if (section.questions[i].quesLangBeans[mockVar.langIndex].quesText.indexOf('<ResponsePlaceHolder>') == -1) {
                         var answers = document.getElementsByName('answers' + section.questions[i].quesId);
                         //keyName =section.questions[i].quesId;
                         for (var j = 0; j < answers.length; j++) {
                             if (answers[j].checked == true) {
                         ...
                         if ($("option:selected", $("#answers" + section.questions[i].quesId)).val() != 'select') {
                             selectedAnswer = $("option:selected", $("#answers" + section.questions[i].quesId)).text() + "<ANS_SEP>";
                             selectedOptId = $("option:selected", $("#answers" + section.questions[i].quesId)).val() + ",";
                         }
                     }
                     if (selectedAnswer != "") {
             ...
             if (section.questions[i].comprehensionId == compreQues || section.questions[i].laqId == compreQues) {
                 if (section.questions[i].quesType == "SA" || section.questions[i].quesType == "COMPREHENSION@@SA" || section.questions[i].quesType == "LA@@SA") {
                     ...
                     if (additionalType !== undefined && additionalType !== "") {
                         ...
                         if ((section.questions[i].quesType == "MSQ") || (section.questions[i].quesLangBeans[mockVar.langIndex].quesText.indexOf('<ResponsePlaceHolder>') == -1)) {
                             var answers = document.getElementsByName('answers' + section.questions[i].quesId);
                             for (var j = 0; j < answers.length; j++) {
                                 if (answers[j].checked == true) {
                                     selectedAnswer = answerTxt(j, answers, selectedAnswer);
     ...
     if (action != "SKIP") {
         if (mockVar.curQuesBean.quesType == "SA" || mockVar.curQuesBean.quesType == "COMPREHENSION@@SA" || mockVar.curQuesBean.quesType == "LA@@SA") {
             if (mockVar.curQuesBean.keyboardType == "FileUpload") {
                 ...
                 if (action != "PREVIOUS") {
                     if (mockVar.curQuesBean.quesLangBeans[mockVar.langIndex].quesText.indexOf('<ResponsePlaceHolder>') == -1) {
                         selectedAnswer = $('#answer').val();
                     } else {
                         selectedAnswer = $('#answer' + mockVar.curQuesBean.quesId).val();
                     }
                     if (selectedAnswer.indexOf('"') > -1) {
                         selectedAnswer = selectedAnswer.replace(/\"/g, '&quot;');
                     }
             ...
             if ((mockVar.curQuesBean.quesType.indexOf('MSQ') != -1) || (mockVar.curQuesBean.quesLangBeans[mockVar.langIndex].quesText.indexOf('<ResponsePlaceHolder>') == -1)) {
                 //var answers = document.getElementsByName('answers'+mockVar.curQuesBean.quesId);
                 //keyName =section.questions[i].quesId;
                 for (var j = 0; j < answers.length; j++) {
                     if (answers[j].checked == true) {
                         selectedAnswer = answerTxt(j, answers, selectedAnswer);
                 ...
                 if ($("option:selected", $("#answers" + mockVar.curQuesBean.quesId)).val() != 'select') {
                     selectedAnswer = $("option:selected", $("#answers" + mockVar.curQuesBean.quesId)).text() + "<ANS_SEP>";
                     selectedOptId = $("option:selected", $("#answers" + mockVar.curQuesBean.quesId)).val() + ",";
                 }
             }
 ...
 function save(ansID, action, quesType) {
     ...
     if (action == "MARK") {
         if (quesStatus == "answered") {
             if (iOAP.secDetails[iOAP.curSection].answered > 0)
                 iOAP.secDetails[iOAP.curSection].answered--;
         } else if (quesStatus == "notanswered") {
             if (iOAP.secDetails[iOAP.curSection].notanswered > 0)
                 iOAP.secDetails[iOAP.curSection].notanswered--;
         }
         ...
         mockVar.curQuesBean.quesParam.isMarked=0;*/
             ...
             if (iOAP.secDetails[iOAP.curSection].marked > 0)
                 iOAP.secDetails[iOAP.curSection].marked--;
         } else if (quesStatus == "markedAndAnswered") {
             if (iOAP.secDetails[iOAP.curSection].markedAndAnswered > 0)
                 iOAP.secDetails[iOAP.curSection].markedAndAnswered--;
         }
         if (quesAnswer != null && quesAnswer != '') {
             iOAP.secDetails[iOAP.curSection].markedAndAnswered++;
         ...
         if (quesStatus == "marked" && iOAP.secDetails[iOAP.curSection].marked > 0) {
             iOAP.secDetails[iOAP.curSection].marked--;
             iOAP.secDetails[iOAP.curSection].notanswered++;
         } else if (quesStatus == "markedAndAnswered" && iOAP.secDetails[iOAP.curSection].markedAndAnswered > 0) {
             iOAP.secDetails[iOAP.curSection].markedAndAnswered--;
             iOAP.secDetails[iOAP.curSection].notanswered++;
         } else if (quesStatus == "answered" && iOAP.secDetails[iOAP.curSection].answered > 0) {
             iOAP.secDetails[iOAP.curSection].answered--;
             iOAP.secDetails[iOAP.curSection].notanswered++;
         }
         mockVar.curQuesBean.quesParam.status = "notanswered";
         ...
         if (ansID == null) {
             if (quesStatus == "answered" && iOAP.secDetails[iOAP.curSection].answered > 0) {
                 iOAP.secDetails[iOAP.curSection].notanswered++;
                 iOAP.secDetails[iOAP.curSection].answered--;
             } else if (quesStatus == "marked" && iOAP.secDetails[iOAP.curSection].marked > 0) {
                 iOAP.secDetails[iOAP.curSection].notanswered++;
                 iOAP.secDetails[iOAP.curSection].marked--;
             } else if (quesStatus == "markedAndAnswered" && iOAP.secDetails[iOAP.curSection].markedAndAnswered > 0) {
                 iOAP.secDetails[iOAP.curSection].notanswered++;
             ...
             if (quesStatus != "answered") {
                 ...
                 if (quesStatus == "markedAndAnswered" && iOAP.secDetails[iOAP.curSection].markedAndAnswered > 0) {
                     iOAP.secDetails[iOAP.curSection].markedAndAnswered--;
                 }
                 if (quesStatus == "notanswered" && iOAP.secDetails[iOAP.curSection].notanswered > 0)
                     iOAP.secDetails[iOAP.curSection].notanswered--;
                 iOAP.secDetails[iOAP.curSection].answered++;
             }
 ...
 function timeOutSubmit() {
     ...
     if (mockVar.storeCandResponse == 1) {
         ...
         if (!isFinalSubmitStarted) {
             $("#sectionSummaryDiv").html(str);
             $('#sectionSummaryDiv').height($(window).height() - jQuery("#header").outerHeight(true));
             $('#group_summary').height($('#sectionSummaryDiv').height() - $('.btnsection').outerHeight(true) - $('.examSummaryHeader').outerHeight(true) - 100);
 ...
 function showQP() {
     ...
     if ($(window).width() < 1000) {
         ...
         showModule('QPDiv');
         $('#QPDiv').css({
             height: (($(window).height() - ($(window).height() * 0.2)) - 25) + 'px',
             width: (($(window).width() - ($(window).width() * 0.2)) - 5) + 'px'
         });
     }
     removingImageHref();
 }
 ...
 function wordCountCheck(e) {
     ...
     var number = 0;
     var quesIdToAppend = mockVar.curQuesBean.quesLangBeans[mockVar.langIndex].quesText.indexOf('<ResponsePlaceHolder>') == -1 ? '' : mockVar.curQuesBean.quesId;
     var matches = $('#answer' + quesIdToAppend).val().match(/\S+/g);
     if (matches) {
         number = matches.length;
     }
     if (number + 1 > mockVar.curQuesBean.alphaWordLimit) {
         if (!(mockVar.curQuesBean.alphaWordLimit == 0)) {
             ...
             $('#answer' + quesIdToAppend).keypress(function(e) {
                 if (e.keyCode != 8) {
                     e.preventDefault();
                 }
             });
         ...
         $('#answer' + quesIdToAppend).unbind('keypress');
         $("#maxalert").text("");
     }
 }
 ...
 function mockScoreCalc() {
     var totalQues = 0,
         ques = '',
         isParentLAQCorrect = false;
     for (var groupNo = 0; groupNo < mockVar.groups.length; groupNo++) {
         var temp_iOAP = mockVar.groups[groupNo],
             grpSectCounter = 0;
         for (var i = 0; i < temp_iOAP.secDetails.length; i++) {
 ...
 function showScoreCard() {
     ...
     for (var groupNo = 0; groupNo < mockVar.groups.length; groupNo++) {
         var totalGrpQues = 0,
             ...
             totalGrpIncorrect = 0,
             totalGrpScore = 0,
             totalGrpNotEvaluated = 0,
             totalGrpMarks = 0,
             grpSectCounter = 0;
         var temp_iOAP = mockVar.groups[groupNo],
             typing_iOAP = mockVar.typingGroup[groupNo];
         if (temp_iOAP.isTypingGroup) {
             if (mockVar.groups.length > 1)
 ...
 function loadLabel() {
     ...
     $('.max_img_ico').attr('title', $(globalXmlvar).find('clickImageToZoom').text());
 }
 ...
 function sendResponseToServlet(action) {
     ...
     /*$.post(
     		mockVar.candResponseUrl,
     		{para : "storeBackup@@BackUpParamSeparator@@"+jsonString+"@@BackUpParamSeparator@@"+"true@@BackUpParamSeparator@@"+authenticationKey},
     		/*{para : "storeBackup@@BackUpParamSeparator@@"+jsonString+"@@BackUpParamSeparator@@true"},
     		function(data) {
     			isfinalSubmit = true;
     			DataError = data;
     ...
     var saveBackUpURL = mockVar.candResponseUrl + "?para=storeBackup@@BackUpParamSeparator@@oldJSON@@BackUpParamSeparator@@true@@BackUpParamSeparator@@" + authenticationKey;
     $.ajax({
         url: saveBackUpURL,
         type: 'POST',
         async: true,
         data: JSON.stringify(mockVar),
         dataType: 'json',
         success: function(data) {
             isfinalSubmit = true;
             DataError = data;
 ...
 function submitOnlineAssessment() {
     if (DataError.ERROR.indexOf('Assessment has been submitted successfully.') != -1) {
         ...
         if (($.cookie("isApp") == "true") ? true : false) {
             window.location.href = "close.html?" + mockVar.orgId + "@@" + mockVar.mockId + "@@" + mockVar.subscribedFor + "#isApp" + ($.cookie("isLauncher") == "true") ? "isLauncher" : "";
         } else
             window.close();
 ...
 function saveBackUp(isRecursionRequired) {
     ...
     if (resumeFirstBackUp)
         extraString = "@@BackUpParamSeparator@@true";
     //$('#pWait').show();
     if (!isFinalSubmitStarted) {
         if (mockVar.storeCandResponse == 1) {
             /*$.post(
             		mockVar.candResponseUrl,
             		{para : "storeBackup@@BackUpParamSeparator@@"+jsonString+"@@BackUpParamSeparator@@"+"false@@BackUpParamSeparator@@"+authenticationKey+extraString},
             		//{para : "storeBackup@@BackUpParamSeparator@@"+jsonString+"@@BackUpParamSeparator@@"+"false"},
             		function(data) {
             			...
             			}else if(data != null && data.ERROR != null && (data.ERROR.indexOf('Invalid Parameters')!=-1)){
             				window.location.href ="error.html?E129"+(($.cookie("isApp")=="true")?"isApp":"")+(($.cookie("isLauncher")=="true")?"isLauncher":"");
             			}
             ...
             var saveBackUpURL = mockVar.candResponseUrl + "?para=storeBackup@@BackUpParamSeparator@@oldJSON@@BackUpParamSeparator@@false@@BackUpParamSeparator@@" + authenticationKey + extraString;
             $.ajax({
                 url: saveBackUpURL,
                 type: 'POST',
                 async: true,
 ...
 function onLoadBackup() {
     //var jsonString = JSON.stringify(mockVar);
     //var URL = '/IBA/MockCandResponseHandler';
     $('#pWait').show();
     if (!isFinalSubmitStarted) {
         $.ajaxSetup({
             async: false
         ...
         /*$.post(
         		...
         		URL,
         		{para : "storeBackup@@BackUpParamSeparator@@"+jsonString+"@@BackUpParamSeparator@@"+"false@@BackUpParamSeparator@@"+authenticationKey+"@@BackUpParamSeparator@@true"},
         		/*{para : "storeBackup@@BackUpParamSeparator@@"+jsonString+"@@BackUpParamSeparator@@true"},
         		function(data) {
         			$.ajaxSetup({
         				async: true
         			...
         			if(data != null && data.ERROR != null && (data.ERROR.indexOf('Invalid Parameters')!=-1)){
         			window.location.href ="error.html?E129"+(($.cookie("isApp")=="true")?"isApp":"")+(($.cookie("isLauncher")=="true")?"isLauncher":"");
         			}
         			else if(data != null && data.ERROR != null){
         				isfinalSubmit = true;
         ...
         var saveBackUpURL = mockVar.candResponseUrl + "?para=storeBackup@@BackUpParamSeparator@@oldJSON@@BackUpParamSeparator@@false@@BackUpParamSeparator@@" + authenticationKey + "@@BackUpParamSeparator@@true";
         $.ajax({
             url: saveBackUpURL,
             type: 'POST',
             async: false,
         ...
         setTimeout(function() {
             if (url.indexOf("quiz.html") > 0 && !isfinalSubmit && !isPlayer && mockVar.storeCandResponse == 1 && mockVar.curQuesBean.keyboardType != 'AudioFile' && mockVar.curQuesBean.keyboardType != 'VideoFile' && mockVar.curQuesBean.keyboardType != 'FileUpload') {
				...
				var candMasterId = $.trim(params[1]).split("@@")[3];*/
                             ...
                             explicitClose = true;
                             // window.close();
                             window.location.href = "error.html?E125" + (($.cookie("isApp") == "true") ? "isApp" : "") + (($.cookie("isLauncher") == "true") ? "isLauncher" : "");
                             /*}*/
                         }, 10000);
                     } else if (allowInterruptions == 'NO') {
                         updateInterruptions(parseInt(noOfInterruptions + ""), "Locked");
                         toErrorPage = true;
                         window.location.href = "error.html?E116#" + errorPageContent + "@@" + mockVar.lockedSubmit + "@@" + (($.cookie("isApp") == "true") ? "isApp" : " ") + (($.cookie("isLauncher") == "true") ? "isLauncher" : "");
                     }
                 }
             } else {
                 isPlayer = false;
 ...
 function InterruptionTimer(interruptionTimer) {
     ...
     interruptionTimeout = setTimeout(function() {
         InterruptionTimer(interruptionTimer - 1);
     }, 1000);
 }
     ...
     try {
         ...
         if (fileType == "XML") {
             ...
             try {
                 ...
                 window.location.href = "error.html?E128" + (($.cookie("isApp") == "true") ? "isApp" : "") + (($.cookie("isLauncher") == "true") ? "isLauncher" : "");
             }
         } else {
     ...
     if (cLaq !== "Claq") {
         for (var i = 0; i < mockVar.curSectionQuestions.length; i++) {
             if (mockVar.curSectionQuestions[i].quesId == jwQuesId) {
                 ...
                 if (!found) {
                     mockVar.curSectionQuestions[i].jwAudioVideo.push({
                         jwplayerId: name,
                         jwplayerPosn: "",
                         noOfReplays: -1
                     });
         ...
         if (mockVar.compreLaqQues.length != 0) {
             for (var i = 0; i < mockVar.compreLaqQues.length; i++) {
                 if (mockVar.compreLaqQues[i].quesId == jwQuesId) {
                     ...
                     if (!found) {
                         mockVar.compreLaqQues[i].jwAudioVideo.push({
                             jwplayerId: name,
                             jwplayerPosn: "",
                             noOfReplays: -1
                         });
 ...
 function scrollToBottom() {
     $('.leftDiv').animate({
         scrollTop: $(".leftDiv").get(0).scrollHeight
     }, 1200);
     return false;
 }
     ...
     questionLevelTimeCounter = setTimeout(function() {
         if (typeof(mockVar.isInstruPage) == "undefined" || mockVar.isInstruPage == 1) {
             ...
             if (isSubmitButtonClicked) {
                 ...
                 isRecursionRequired = true;
                 if (typeof(mockVar.groups[groupIndex].secDetails[sectionIndex].questions[questionIndex].timespent) == "undefined") {} else if (mockVar.groups[groupIndex].secDetails[sectionIndex].groupAllQuestions != 'undefined' && mockVar.groups[groupIndex].secDetails[sectionIndex].groupAllQuestions == 'true') {
 ...
 function pauseJwAudioVideoOnResume() {
             ...
             for (var i = 0; i < mockVar.curSectionQuestions.length; i++) {
                 if (mockVar.curSectionQuestions[i].jwAudioVideo.length != 0) {
                     for (var j = 0; j < mockVar.curSectionQuestions[i].jwAudioVideo.length; j++) {
                         if (mockVar.curSectionQuestions[i].jwAudioVideo[j].jwplayerId == $(this).attr('id')) {
                             if (mockVar.curSectionQuestions[i].jwAudioVideo[j].jwplayerPosn != undefined && mockVar.curSectionQuestions[i].jwAudioVideo[j].jwplayerPosn != 0 && mockVar.curSectionQuestions[i].jwAudioVideo[j].jwplayerPosn != "") {
                                 if (mockVar.curSectionQuestions[i].autoplay == "false") {
             ...
             for (var i = 0; i < mockVar.compreLaqQues.length; i++) {
                 if (mockVar.compreLaqQues[i].jwAudioVideo.length != 0) {
                     for (var j = 0; j < mockVar.compreLaqQues[i].jwAudioVideo.length; j++) {
                         if (mockVar.compreLaqQues[i].jwAudioVideo[j].jwplayerId == $(this).attr('id')) {
                             if (mockVar.compreLaqQues[i].jwAudioVideo[j].jwplayerPosn != undefined && mockVar.compreLaqQues[i].jwAudioVideo[j].jwplayerPosn != 0 && mockVar.compreLaqQues[i].jwAudioVideo[j].jwplayerPosn != "") {
                                 if (mockVar.compreLaqQues[i].autoplay == "false") {
                                     jwplayer($(this).attr('id')).pause();
                                 }
                                 break;
                             }
 ...
 $(function() {
     if (/iPhone|iPod|iPad/.test(navigator.userAgent)) {
         $('.mContent,#commonPageInstruction').css({
             overflow: 'auto',
 ...
 function addMediaFiles(answerTxt, str1, answers, i) {
     ...
     if (str1.indexOf("imageZoom") > -1) {
         ...
         if (i == "add") {
             ...
             $(answers[i]).parent().next('span').children().each(function() {
                 var src = "";
                 if (this.hasAttribute("class") && this.getAttribute("class").indexOf("ans") > -1) {
                     src = $(this).children('span').children('a').children('img').attr('src');
                     if (src.indexOf("tkcimages") == -1)
                         images = images + src + "##@@";
                     else
                         images = images + src.split('tkcimages/')[1] + "##@@";
                     imageCount++;
                 } else {
                     var element = this.getElementsByClassName("ans");
